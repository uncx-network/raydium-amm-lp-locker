//!The above traits set a foundation to impl a more efficient zero-copy compared to anchor zero
//! copy impl which costs an allocation in the account converstion to account info, this impl works
//! on ref avoiding the allocation cost For Our purpose we will change the casting logic to make it
//! work with Resizable Data Structures such as a Vec Taking Inspiration from the solana ([w])
...
// NOTE: would love to use solana's ReadableAccount, but that's in solana_sdk -- unavailable for
// programs
